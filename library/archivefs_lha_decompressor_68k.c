...
{
	const uint32_t windowEnd=state->windowStart+state->windowSize;
	if (state->reqOffset>=windowEnd)
		return 1;
	const uint32_t windowOfs=state->reqOffset-state->windowStart;
	const uint32_t windowMax=state->windowSize-windowOfs;
	uint32_t len=state->reqLength;
	if (len>windowMax)
		len=windowMax;
...
static int writeCallback(REG(a0,void* context),REG(a1,uint8_t* buf),REG(d0,uint32_t size))
{
	struct archivefs_lhaDecompressState* state=context;
